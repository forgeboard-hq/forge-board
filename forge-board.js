   ...
   ================================================================ */
      const SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbylNKVDO56Vrew4TffammlA9GkxmoBq_7Ktx7U60PlLk6oAgwRbxrXw5Gt_bogTrVVZqQ/exec';
      ...
      const ADJ = [
        'faithful', 'humble', 'gentle', 'patient', 'gracious', 'steadfast', 'righteous', 'blessed',
        'redeemed', 'joyful', 'merciful', 'devoted', 'upright', 'contrite', 'anointed', 'beloved',
        'chosen', 'kind', 'meek', 'pure', 'valiant', 'watchful', 'ardent', 'tender',
        'hopeful', 'grateful', 'radiant', 'abiding', 'lowly', 'earnest', 'fervent', 'peaceful',
        'holy', 'mended', 'ransomed', 'called', 'kindled', 'quiet', 'sincere', 'trusting',
        'willing', 'prayerful', 'gathered', 'restored', 'loving', 'truthful', 'honest', 'worthy',
        'noble', 'wise', 'brave', 'bold', 'zealous', 'cheerful', 'thankful', 'mighty',
        'shining', 'golden', 'crowned', 'anchored', 'rooted', 'ready', 'awakened', 'renewed',
        'forgiven', 'mindful', 'eternal', 'living', 'burning', 'rising', 'soaring', 'blooming',
        'flowing', 'guarded', 'guided', 'sheltered', 'sealed', 'promised', 'graceful', 'tranquil',
        'serene', 'cherished', 'honored', 'revered', 'sacred', 'hallowed', 'divine', 'heavenly',
        'saintly', 'devout', 'reverent', 'obedient', 'yielded', 'enduring', 'abounding', 'overflowing',
        'flourishing', 'fruitful', 'bountiful', 'generous', 'victorious', 'triumphant',
      ];
      const NOUN = [
        'lamb', 'dove', 'lion', 'shepherd', 'vine', 'olive', 'cedar', 'lily',
        'sparrow', 'eagle', 'ram', 'stag', 'palm', 'reed', 'well', 'lamp',
        'seed', 'harvest', 'manna', 'ark', 'altar', 'psalm', 'scroll', 'beacon',
        'ember', 'vessel', 'branch', 'fig', 'wheat', 'fountain', 'anchor', 'banner',
        'raven', 'dawn', 'pilgrim', 'sower', 'watchman', 'servant', 'fold', 'stream',
        'mantle', 'candle', 'fisher', 'deer', 'rock', 'river', 'mountain', 'valley',
        'garden', 'vineyard', 'orchard', 'meadow', 'pasture', 'spring', 'brook', 'oak',
        'myrtle', 'cypress', 'almond', 'pomegranate', 'honey', 'bread', 'oil', 'wine',
        'salt', 'light', 'flame', 'torch', 'star', 'sun', 'moon', 'cloud',
        'rain', 'dew', 'wind', 'fire', 'stone', 'pearl', 'crown', 'robe',
        'sandal', 'staff', 'rod', 'shield', 'sword', 'trumpet', 'harp', 'cymbal',
        'incense', 'temple', 'gate', 'tower', 'wall', 'sanctuary', 'throne', 'refuge',
        'haven', 'shelter', 'cornerstone', 'foundation',
      ];
      ...
      let theme = localTheme ? localTheme :window.matchMedia('(prefers-color-scheme: dark)').matches
        ? 'dark'
        : 'light';
      applyTheme();
      ...
      function showToast(msg, kind, opts) {
        ...
        if (!opts.sticky)
          setTimeout(() => dismissToast(el), opts.duration || 2200);
        return el;
      }
   ...
   storage-blocked environment just falls back to no persistence.   */
      ...
      $('claimBtn').onclick = async () => {
        ...
        if (res.ok) {
          startSession(res.codename || candidate, $('pinClaim').value);
          await fetchBoard();
        } else {
      ...
      $('loginBtn').onclick = async () => {
        ...
        if (!codename || !/^\d{4,8}$/.test(pin)) {
          setMsg(
            $('claimMsg'),
            'Enter your codename and 4–8 digit PIN.',
            'err'
          );
          return;
        }
      ...
      function startSession(codename, pin) {
      ...
      async function toggleCell(taskId) {
        ...
        if (res.ok) {
          ...
          if (/wrong codename or pin/i.test(res.error || ''))
            $('signOutBtn').click();
        }
        renderBoard();
      }
    ...
    <thead>
      <tr>
        ...
        const adminDoneN = data.tasks.reduce(
          (s, t) => s + (t.adminDone ? 1 : 0),
          0
        );
    ...
    <tr class="admin-row">
      ...
      <th class="who"><div class="who-name">${esc(code)}${you ? "<span class='you-tag'>YOU</span>" : ''}</div>${chip(nDone)}</th>${cells}</tr>`;
        ...
        wrap
          ...
          .forEach(
            (b) =>
              (b.onclick = () => openTask(b.closest('.task-th').dataset.task))
          );
        wrap
